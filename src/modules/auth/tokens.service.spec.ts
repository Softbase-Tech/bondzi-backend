...
describe('TokensService', () => {
  ...
  describe('issuePair', () => {
    it('upserts the session for the user (atomic, single-device enforcement)', async () => {
      ...
      // race two simultaneous logins. The fix is a single INSERT ... ON
      // CONFLICT (user_id) DO UPDATE, written as raw SQL because
      // rotation-grace requires moving the pre-update
      // `device_sessions.refresh_token_jti` into
      // `previous_refresh_jti` in the same statement — a column-to-
      // column move that TypeORM's high-level orUpdate() can't express.
      // Verify we take that path (not the legacy delete + insert) and
      // that the parameter binding is correct.
      ...
      // The grace-move must be present: previous jti receives the
      // pre-update refresh_token_jti and expires after a positive
      // interval. Without this, force-quit-mid-refresh regresses.
      expect(sql).toMatch(
        /previous_refresh_jti = device_sessions\.refresh_token_jti/,
      );
      expect(sql).toMatch(/previous_jti_expires_at = now\(\) \+/);
      // Parameter binding: user, deviceId, deviceName (null OK),
      // refreshJti, ip, and the grace interval string.
      expect(params[0]).toBe('user-1');
      expect(params[1]).toBe('d1');
      // params[3] is the freshly-minted refresh JTI — any non-empty
      // string is fine, the value is generated by randomUUID.
